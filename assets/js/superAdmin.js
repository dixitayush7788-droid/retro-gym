...
const SUPABASE_ANON_KEY = (typeof window !== 'undefined' && window.NEXUS_CONFIG?.SUPABASE_ANON_KEY) || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmF0ZSI6Inpmdmt2cmh1b3Z2YmZicnV0cHBoIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODcxMzIyODQsImV4cCI6MjEwMjcwODI4NH0.M-WK1bgZDLXcuMTldMSwptx5XRpRnLAi-BxMFEoph4U';
...
export function withTimeout(promise, timeoutMs = 7000, operationName = 'Operation') {
  ...
  const timeoutPromise = new Promise((_, reject) => {
    timer = setTimeout(() => reject(new Error(`[TIMEOUT] ${operationName} timed out after ${timeoutMs}ms`)), timeoutMs);
  });
...
export async function onboardGymNode({
  gymName, slug, phone, email, adminPin = '1234', upi,
  ...
  } catch (ctxErr) { console.warn('[ONBOARDING] User context RPC check note:', ctxErr); }
  ...
  // Auth provisioning is deliberately server-side. The browser never receives or stores
  // a generated owner password and never calls auth.admin.* APIs.
  const { data, error } = await supabase.functions.invoke('nexus-create-gym-owner', {
    body: {
      gym_name: gymName,
      slug: cleanSlug,
      owner_phone: phone,
      ...
      owner_upi_id: upi,
      admin_pin: String(adminPin || '1234'),
  ...
  return {
    success: true,
    gym: data.gym || { id: data.gym_id, name: gymName, slug: cleanSlug, phone, email },
    gymId: data.gym_id,
    authUserId: data.owner_user_id || data.owner_user_id,
    email,
    password: '',
    ...
    gymName,
    phone,
    authNotice: data.auth_notice || 'Invitation email sent. Owner must open the email and set a password before first login.',
    invitationSent: data.invitation_sent === true
  };
}
...
export function checkIsSuperAdmin(user, context) {
  ...
  if (context) {
    ...
    if (Array.isArray(context.roles)) return context.roles.some(r => {
  ...
  if (user?.app_metadata) {
    ...
    if (String(appMeta.role || '').toUpperCase() === 'SUPER_ADMIN') return true;
...
export async function initSuperAdmin() {
  ...
  bootstrapPromise = (async () => {
    try {
      ...
      let context = null;
      try {
        const { data: rpcContext, error: rpcErr } = await withTimeout(supabase.rpc('rpc_get_current_user_context'), 5000, 'rpc_get_current_user_context');
        if (rpcErr) return { error: 'SESSION_ERROR', message: `Unable to verify Super Admin role: ${rpcErr.message || 'role context RPC failed'}` };
        context = rpcContext;
      } catch (rpcErr) {
        return { error: rpcErr?.message?.includes('TIMEOUT') ? 'TIMEOUT' : 'SESSION_ERROR', message: rpcErr?.message || 'Unable to verify Super Admin role.' };
      }
      ...
      return { error: error.message?.includes('TIMEOUT') ? 'TIMEOUT' : 'BOOT_ERROR', message: error.message || 'Authentication initialization error' };
...
// super-admin.html currently uses an inline handler that checks this bridge first.
// Expose only the safe application-level onboarding function; no Auth Admin API is exposed.
