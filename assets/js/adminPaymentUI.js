...
(() => {
  ...
  function ensureModal() {
    ...
    el.innerHTML = `
      <div class="w-full max-w-md bg-[#0d0d12] border border-emerald-400/40 rounded-t-3xl sm:rounded-3xl p-5 shadow-2xl space-y-4">
        ...
        <div><label class="block text-[10px] font-mono uppercase tracking-wider text-zinc-400 mb-1">Actual Amount Collected (₹) *</label><input id="nexus-payment-amount" type="number" min="1" max="100000000" step="1" inputmode="decimal" class="w-full bg-black/40 border border-emerald-400/30 focus:border-emerald-400 rounded-2xl p-3.5 text-xl font-bold text-emerald-300 outline-none" placeholder="Enter actual amount" /><p class="text-[10px] text-zinc-500 mt-1">Enter the exact amount received from this member. Plan price is only a suggestion.</p></div>
  ...
  async function savePayment() {
    ...
    try {
      ...
      if (typeof window.openAthleteDossier === 'function') await window.openAthleteDossier(member.normalized_phone, false);
  ...
  async function openPayment(phone) {
    ensureModal();
    try {
      const resolved = await resolveMember(phone); state = resolved;
      const { member, membership } = resolved;
      const suggested = Number(membership.plans?.price || 0);
      document.getElementById('nexus-payment-title').textContent = 'RECORD MEMBER PAYMENT';
