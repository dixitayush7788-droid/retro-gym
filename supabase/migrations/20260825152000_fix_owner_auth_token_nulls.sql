-- Fix GoTrue password-login failures for users provisioned directly by the
-- onboarding RPC. GoTrue scans several auth.users token columns as strings;
-- NULL values can cause: `error finding user: ... confirmation_token:
-- converting NULL to string is unsupported` and return HTTP 500.
...
SET
  ...
  updated_at = now()
WHERE raw_app_meta_data->>'role' = 'GYM_OWNER'
  AND (
    confirmation_token IS NULL OR recovery_token IS NULL OR email_change_token_new IS NULL
    OR email_change IS NULL OR email_change_token_current IS NULL OR reauthentication_token IS NULL
    OR phone_change IS NULL OR phone_change_token IS NULL
  );

-- Future onboarding users must be inserted with GoTrue-compatible empty token
-- strings rather than NULLs. Keep the existing schema/search-path/security
-- contract from the preceding onboarding migration; only the auth insert is
-- changed here.
...
BEGIN
  IF v_caller_id IS NOT NULL THEN
    SELECT EXISTS (SELECT 1 FROM public.user_roles WHERE user_id = v_caller_id AND role = 'super_admin'::public.app_role_type) INTO v_is_super_admin;
    IF NOT v_is_super_admin THEN
      SELECT coalesce((u.raw_app_meta_data->>'is_super_admin')::boolean, false) OR (u.raw_app_meta_data->>'role' = 'SUPER_ADMIN')
      INTO v_is_super_admin FROM auth.users u WHERE u.id = v_caller_id;
    END IF;
    IF NOT v_is_super_admin THEN
  ...
  END IF;
  IF coalesce(p_saas_fee, 2499) < 0 OR coalesce(p_validity_days, 365) <= 0 THEN
    RAISE EXCEPTION 'Invalid SaaS billing/validity values.' USING ERRCODE = '22023';
  END IF;
  ...
  END IF;
  SELECT u.id INTO v_auth_user_id FROM auth.users u WHERE lower(u.email) = v_email AND u.deleted_at IS NULL LIMIT 1;
  ...
  INSERT INTO auth.users (
    ...
    v_email, v_encrypted_pw, v_now, '', '', '', '', '', '', '', '', false, false,
  ...
  VALUES (extensions.gen_random_uuid(),v_auth_user_id,jsonb_build_object('sub',v_auth_user_id::text,'email',v_email),'email',v_email,v_now,v_now,v_now);

  INSERT INTO public.gyms (
    name, slug, phone, email, created_by, is_active, owner_upi_id, monthly_saas_fee,
    validity_days, pricing_plans, feature_gates, admin_pin_hash, pin_failed_attempts, pin_locked_until, deleted_at
  ) VALUES (
    ...
    coalesce(p_pricing,'{}'::jsonb), coalesce(p_feature_gates,'{}'::jsonb), extensions.crypt(v_pin,extensions.gen_salt('bf')),0,NULL,NULL
  ...
  RETURN jsonb_build_object(
    ...
    'gym',jsonb_build_object('id',v_gym_row.id,'name',v_gym_row.name,'slug',v_gym_row.slug,'phone',v_gym_row.phone,
      'email',v_gym_row.email,'is_active',v_gym_row.is_active,'owner_upi_id',v_gym_row.owner_upi_id,
      'monthly_saas_fee',v_gym_row.monthly_saas_fee,'validity_days',v_gym_row.validity_days)
  )::json;
END;
$$;
