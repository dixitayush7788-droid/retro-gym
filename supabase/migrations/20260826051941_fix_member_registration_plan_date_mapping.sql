-- Fix member registration failure caused by assigning integer plan duration_days to a DATE variable.
-- Also resolves the real plan duration safely (including 365-day 12-month plans).
...
BEGIN
  ...
  IF EXISTS (
    ...
    WHERE m.gym_id = v_gym_id
      AND m.normalized_phone = v_clean
      AND m.deleted_at IS NULL
  ) THEN
    RAISE EXCEPTION 'MEMBER_PHONE_ALREADY_EXISTS';
  END IF;

  -- The live plans table stores 12 months as 365 days; shorter plans use 30-day blocks.
  v_target_days := CASE WHEN p_plan_months = 12 THEN 365 ELSE p_plan_months * 30 END;
  ...
  ORDER BY abs(p.duration_days - v_target_days), p.price ASC, p.created_at ASC
  LIMIT 1;

  IF v_plan_id IS NULL OR v_duration_days IS NULL OR abs(v_duration_days - v_target_days) > 7 THEN
    RAISE EXCEPTION 'PLAN_NOT_FOUND_FOR_DURATION';
  END IF;

  INSERT INTO public.members(
    gym_id, full_name, phone, age, address, baseline_photo_path,
    referral_code, referred_by_member_id, is_active,
  ...
  IF coalesce(p_fees_paid,0) > 0 THEN
    INSERT INTO public.payments(
      gym_id, member_id, membership_id, amount, payment_method,
      payment_status, transaction_ref, recorded_by_user_id, notes, created_at
    )
    VALUES(
      v_gym_id,
      v_member_id,
      v_membership_id,
      p_fees_paid,
      'other'::public.payment_method_type,
      'completed'::public.payment_status_type,
      NULL,
      v_uid,
      'Legacy registration payment',
      CURRENT_TIMESTAMP
    );
  END IF;
  ...
  SELECT EXISTS (
    ...
    WHERE rl.gym_id = v_gym_id
      AND rl.referred_member_id = v_member_id
